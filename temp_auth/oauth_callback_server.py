...
class MinimalOAuthServer:
    ...
    def _setup_callback_route(self):
        ...
        async def oauth_callback(request: Request):
            ...
            # Verify that the state token returned by Google matches the one we set in the cookie
            # (set by app/api/auth/oauth-start/route.ts)
            cookie_state = request.cookies.get("oauth_state")
            
            # Allow skipping cookie check only if state matches a known "cli-initiated" pattern (optional, strict for now)
            # If cookie is missing, it might be a direct access or session timeout.
            if not cookie_state:
                logger.warning(f"OAuth CSRF Warning: No oauth_state cookie found. State from query: {state}")
                # For strict security, we should reject. However, to avoid breaking existing CLI flows 
                # that might not use the browser cookie mechanism (e.g. strict stdio), 
                # we might need to be careful.
                # Assuming the User Request implies web flow vulnerability.
                # IMPLEMENTATION: Reject if state doesn't match cookie, OR if cookie missing but state present.
                if state: # State is required by Google Auth usually
                     # If we are in a pure CLI context, maybe cookies aren't used?
                     # But this server handles the browser callback. The browser *should* have the cookie.
                     return create_error_response("Security Error: Missing state cookie. Please try again.")

            if state != cookie_state:
                logger.warning(f"OAuth CSRF Attack Detected? Query state: {state}, Cookie state: {cookie_state}")
                return create_error_response("Security Error: State mismatch. Possible CSRF attempt.")
    ...
    def start(self) -> tuple[bool, str]:
        ...
        try:
             # If port is 0, we bind to a random port
            with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
                s.bind((hostname, self.port))
                # Update port if we chose random
                if self.port == 0:
                    self.port = s.getsockname()[1]
                    logger.info(f"Dynamic port selected: {self.port}")
        except OSError:
...
def ensure_oauth_callback_available(
    ...
    if transport_mode == "streamable-http":
        ...
        if not _minimal_oauth_server.is_running:
            ...
            if success:
                # Update global config with the actual port (in case it was dynamic)
                actual_port = _minimal_oauth_server.port
                from auth.oauth_config import get_oauth_config
                get_oauth_config().update_port(actual_port)
