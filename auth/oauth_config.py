...
class OAuthConfig:
    ...
    def update_port(self, port: int) -> None:
        """
        Update the port and dependent URLs (base_url, redirect_uri).
        Used when the callback server binds to a dynamic port (e.g. port 0).
        """
        self.port = port
        self.base_url = f"{self.base_uri}:{self.port}"
        # Recalculate redirect_uri if it wasn't explicitly set via env var
        if not os.getenv("GOOGLE_OAUTH_REDIRECT_URI"):
            self.redirect_uri = f"{self.base_url}/oauth2callback"
            self.redirect_path = self._get_redirect_path(self.redirect_uri)
        
        # Re-apply environment variables for FastMCP
        self._apply_fastmcp_google_env()
